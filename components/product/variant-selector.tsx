...
export function VariantSelector({ options, variants }: { options: ProductOption[]; variants: ProductVariant[] }) {
 ...
 useEffect(() => {
  ...
  const preferredSize = sizeOption.values.find((value) => value === "M") ?? sizeOption.values[0] ?? "M";

  const firstAvailableSize = sizeOption.values.find((value) => combinations.some((combination) => combination.size === value && combination.availableForSale)) ?? preferredSize;

  const params = new URLSearchParams(searchParams.toString());
  params.set("size", firstAvailableSize);
  router.replace(`?${params.toString()}`, { scroll: false });
 }, [options, combinations, searchParams, router]);
      ...
      return (
       <button
        ...
        className={clsx("flex min-w-[48px] items-center justify-center rounded-full border bg-neutral-100 px-2 py-1 text-sm dark:border-neutral-800 dark:bg-neutral-900", {
         ...
         "ring-1 ring-transparent transition duration-300 ease-in-out hover:ring-neutral-400": !isActive && isAvailableForSale,
