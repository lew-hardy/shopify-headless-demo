...
export function Gallery({
  ...
  const imageIndex =
    Number.isNaN(parsedIndex) || parsedIndex < 0 || parsedIndex >= images.length
      ? 0
      : parsedIndex;
    ...
    <>
            ...
            <>
              ...
              <div className="absolute bottom-4 z-10 flex w-full justify-center">
                <div className="mx-auto flex h-11 items-center rounded-full border border-white bg-neutral-50/80 text-neutral-500 backdrop-blur-sm dark:border-black dark:bg-neutral-900/80">
                  <button
                    formAction={() => updateImage(previousImageIndex)}
                  ...
                  <button
                    formAction={() => updateImage(nextImageIndex)}
      ...
      {isLightboxOpen && (
        ...
        >
          ...
          <button
            onClick={() => setIsLightboxOpen(false)}
            className="absolute right-6 top-6 z-20 text-white"
          ...
          {images.length > 1 && (
            <>
              <button
                type="button"
                onClick={goPrevious}
                className="absolute left-4 z-20 rounded-full bg-black/40 p-3 text-white transition hover:bg-black/60"
              ...
              <button
                type="button"
                onClick={goNext}
                className="absolute right-4 z-20 rounded-full bg-black/40 p-3 text-white transition hover:bg-black/60"
          ...
          <div className="pointer-events-none relative z-10 flex h-full w-full items-center justify-center p-6">
            <Image
              ...
              width={1600}
              height={1600}
              className="pointer-events-auto max-h-[90vh] w-auto object-contain"
            />
          </div>
        </div>
      )}
